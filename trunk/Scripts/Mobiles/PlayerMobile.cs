	...
	public partial class PlayerMobile : Mobile, IHonorTarget
	{
		...
		*/
		...
		public override void Resurrect()
		{
			...
			#region GeNova: KR Support
            if (this.NetState != null && (this.NetState.IsKRClient || this.NetState.IsSAClient))
            {
                List<BaseHealer> listHealers = new List<BaseHealer>();
                List<BaseQuester> listQuesters = new List<BaseQuester>();
                foreach (Mobile m_mobile in World.Mobiles.Values)
                {
                    BaseQuester mQuester = m_mobile as BaseQuester;
                    if (mQuester != null)
                ...
                foreach (BaseQuester quester in listQuesters)
                {
            ...
            if (this.Party is Server.Engines.PartySystem.Party)
            {
                ...
                if (party != null)
                {
                    foreach (Server.Engines.PartySystem.PartyMemberInfo info in party.Members)
                    {
                        ...
                        if (m.NetState != null && (m.NetState.IsKRClient || m.NetState.IsSAClient) && m != this)
